...
Now I will create a logistic regression ot predict electric heat.  First I split my data into two groups:  a training set and a test set (framework  borrowed from datacamp).  I will use this baseline for comparing to later with a full cross-validation.
...
After some iteration, N = 0.5, 0.6 and 0.4 give similar answers, and N = 0.75 is slightly worse, so we will  go with N = 0.5, where the model predicted the correct outcome 1276 of 1652 customers, an accuracy rate of 77%.


  Model refinement                                 
====================================

Many of the monthly variables are not significant.  This is likely because the impact of electric heating affects some months similarly and they would therfore be highly correlated.  WI will atempt to build a model with only two usage factors.  One for winter load and one for summer load.
...
We will do 10 fold cross validation, with most of the structure adapted from Datacamp.
...
data5$pred <- predict(lm(cty ~ hwy, data = mpg))
